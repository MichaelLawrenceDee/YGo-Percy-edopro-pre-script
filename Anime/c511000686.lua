...
function c511000686.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,c511000686.filter2,tp,LOCATION_EXTRA,0,1,1,nil,tc:GetRank()+1,e,tp,tc):GetFirst()
	g1:SetMaterial(Group.FromCards(tc))
	Duel.SpecialSummon(g1,SUMMON_TYPE_XYZ,tp,tp,false,false,POS_FACEUP_ATTACK)
	local g2=Duel.SelectMatchingCard(tp,c511000686.filter2,tp,LOCATION_EXTRA,0,1,1,nil,tc:GetRank()+2,e,tp,tc):GetFirst()
