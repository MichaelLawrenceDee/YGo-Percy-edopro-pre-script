...
function c511015101.filter2(c,e,tp,m,f)
	...
	if e:IsHasType(EFFECT_TYPE_ACTIVATE) and mc:IsCanBeFusionMaterial(c) and m:IsExists(Card.IsCode,1,nil,47198668) and c511015101[1]:IsExists(c511015101.cfilter,1,nil,c,tp) then
		...
		local res=m:IsExists(c511015101.fusfilter2,1,nil,c,m,tp)
		e1:Reset()
		m:RemoveCard(mc)
		return res
	end
...
function c511015101.fusfilter2(c,fc,m,tp)
	if not c:IsCode(47198668) then return false end
	return fc:CheckFusionMaterial(m,c,tp)
end
function c511015101.cfilter(c,fc,tp)
	c511015101[0]:AddCard(c)
	local res=fc:CheckFusionMaterial(c511015101[0],c,tp)
	...
	if sg1:GetCount()>0 or (sg2~=nil and sg2:GetCount()>0) then
		...
		if sg1:IsContains(tc) and (sg2==nil or not sg2:IsContains(tc) or not Duel.SelectYesNo(tp,ce:GetDescription())) then
			...
			if not tc:CheckFusionMaterial(mg1,nil,tp) or (e:IsHasType(EFFECT_TYPE_ACTIVATE) and c:IsCanBeFusionMaterial(tc) 
				...
				local matc=mg1:FilterSelect(tp,c511015101.fusfilter2,1,1,nil,tc,mg1,tp):GetFirst()
				mat1=Duel.SelectFusionMaterial(tp,tc,mg1,matc,tp)
			else
				mat1=Duel.SelectFusionMaterial(tp,tc,mg1,nil,tp)
			end
			tc:SetMaterial(mat1)
			...
			if not tc:CheckFusionMaterial(mg2,nil,tp) or (e:IsHasType(EFFECT_TYPE_ACTIVATE) and c:IsCanBeFusionMaterial(tc) and mg2:IsExists(Card.IsCode,1,nil,47198668) 
				...
				c:RegisterEffect(e1)
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
				local matc=mg2:FilterSelect(tp,c511015101.fusfilter2,1,1,nil,tc,mg2,tp):GetFirst()
				mat2=Duel.SelectFusionMaterial(tp,tc,mg2,matc,tp)
			else
				mat2=Duel.SelectFusionMaterial(tp,tc,mg2,nil,tp)
			end
