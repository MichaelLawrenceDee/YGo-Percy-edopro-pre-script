...
function c511002583.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=Duel.SelectMatchingCard(tp,c511002583.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil,e,tp)
...
function c511002583.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return not Duel.IsPlayerAffectedByEffect(tp,59822133) and (Duel.GetLocationCount(tp,LOCATION_MZONE)>1 
		or (Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and not Duel.IsPlayerAffectedByEffect(tp,69832741))) end
