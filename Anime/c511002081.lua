...
function c511002081.synchk(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		...
		if tc.synchro_type==1 then
			...
			e1:SetCondition(aux.SynCondition(table.unpack(c.synchro_parameters),aux.FilterBoolFunction(Card.IsHasEffect,511002081)))
			e1:SetTarget(aux.SynTarget(table.unpack(c.synchro_parameters),aux.FilterBoolFunction(Card.IsHasEffect,511002081)))
