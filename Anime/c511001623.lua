...
function c511001623.filter(c)
	if not c:IsType(TYPE_XYZ) or not c:IsAbleToRemove() then return false end
...
function c511001623.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.IsExistingTarget(c511001623.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,3,nil) 
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 
		and Duel.IsExistingMatchingCard(c511001623.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectTarget(tp,c511001623.filter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,3,3,nil)
