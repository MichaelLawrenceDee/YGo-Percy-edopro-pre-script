...
function c511009372.initial_effect(c)
	...
	--fusion material
	aux.AddFusionProcMix(c,true,true,511009366,c511009372.fusfilter)
	c:EnableReviveLimit()	
	--cannot be destroyed
...
end--Master Spirit Tech Force - Pendulum Ruler
--fixed and clean up by Larry126
function c511009372.initial_effect(c)
    aux.EnablePendulumAttribute(c,false)
    --fusion material
    aux.AddFusionProcMix(c,true,true,511009366,c511009372.fusfilter)
...
function c511009372.checkop(e,tp,eg,ep,ev,re,r,rp)
    ...
    while tc do
        if tc:IsFaceup() and tc:IsType(TYPE_MONSTER) then
            tc:RegisterFlagEffect(c511009372,RESET_EVENT+0x1fe0000,0,1)
        end
        tc=eg:GetNext()
    end
end
...
function c511009372.effilter(c)
    return c:IsFaceup() and c:IsSummonType(SUMMON_TYPE_PENDULUM) and c:IsPreviousLocation(LOCATION_HAND)
        and c:IsSetCard(0x1414) and c:GetFlagEffect(511009372)==0 and c:IsReleasableByEffect()
end
