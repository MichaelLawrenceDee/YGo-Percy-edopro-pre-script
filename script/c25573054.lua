...
function c25573054.chk(c,sg,tp)
	return c:IsCode(57116033) and c:IsLocation(LOCATION_ONFIELD) and sg:IsExists(Card.IsLocation,2,c,LOCATION_HAND) 
		and Duel.IsExistingMatchingCard(c25573054.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,sg)
end
...
function c25573054.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		if e:GetLabel()==1 then
			...
			return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and Duel.IsExistingMatchingCard(c25573054.spfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil)
		end
	end
