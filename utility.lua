				...
				if g:IsExists(Auxiliary.SynchroCheckFilterChk,1,nil,f1,f2,sub1,sub2) then
					...
					while ntsg:GetCount()<max2 do
						...
						if tune then
							...
							and ntg:Filter(Auxiliary.SynchroCheckP32,sg,g,tsg,ntsg,sg,f2,sub2,min2,max2,req2,reqct2,reqm,lv,c,tp,smat,pg,mgchk):GetCount()>0)
							or (not mgchk and Duel.GetCurrentChain()<=0 and tsg:GetCount()==0) then
									cancel=true
							end
					...
					while ntsg:GetCount()<max2 do
						...
						if tune then
							...
							and ntg:Filter(Auxiliary.SynchroCheckP42,sg,ntg,tsg,ntsg,sg,min2,max2,req2,reqct2,reqm,lv,c,tp,smat,pg,mgchk):GetCount()>0)
							or (not mgchk and Duel.GetCurrentChain()<=0 and tsg:GetCount()==0) then
								cancel=true
							end
...
function Auxiliary.AddFusionProcMix(c,sub,insf,...)
	...
	for i=1,#val do
		if type(val[i])=='function' then
			...
			local addmat=true
			if sub then
				fun[i]=function(c,fc,sub) return c:IsFusionCode(val[i]) or (sub and c:CheckFusionSubstitute(fc))
				or c:IsHasEffect(511002961) end
			else
...
function Auxiliary.FOperationMix(insf,sub,...)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				while sg:GetCount()<#funs do
					...
					local tc=Group.SelectUnselect(mg:Filter(Auxiliary.FSelectMix,sg,tp,mg,sg,c,sub,chkf,table.unpack(funs)),sg,p,false,false,#funs,#funs)
...
function Auxiliary.FOperationMixRep(insf,sub,fun1,minc,maxc,...)
	...
	return	function(e,tp,eg,ep,ev,re,r,rp,gc,chkfnf)
				...
				while sg:GetCount()<maxc+#funs do
					...
					local cancel=Auxiliary.FCheckMixRepGoal(tp,sg,c,sub,chkf,fun1,minc,maxc,table.unpack(funs))
