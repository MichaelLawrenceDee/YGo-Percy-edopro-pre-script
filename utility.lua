...
function Auxiliary.XyzRecursionChk1(c,mg,xyz,tp,min,max,minc,maxc,matg,ct,matct,mustbemat)
	...
	if xct>=min and xmatct>=minc then
		...
		if ok and tg:IsExists(Auxiliary.FieldChk,1,nil,tp) then return true end
	end
	local retchknum={0}
...
function Auxiliary.XyzCondition(f,lv,minc,maxc,mustbemat)
	...
	return	function(e,c,og,min,max)
				...
				if (not min or sg:GetCount()>=min) and sg:GetCount()>=minc and sg:IsExists(Auxiliary.FieldChk,1,nil,tp) then return true end
				if mustbemat then
