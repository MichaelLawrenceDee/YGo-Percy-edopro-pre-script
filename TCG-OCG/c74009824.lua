...
function c74009824.exfilter(c,g,fc,sumtype,tp)
	return c:IsFaceup() and c:IsCanBeFusionMaterial(fc) and not g:IsContains(c) and (c74009824.ffilter1(c) or c74009824.ffilter2(c,fc,sumtype,tp))
end
function c74009824.ffilter(c,fc,sumtype,tp)
	return c:IsCanBeFusionMaterial(fc) and (c74009824.ffilter1(c) or c74009824.ffilter2(c,fc,sumtype,tp))
end
function c74009824.filterchk(c,tp,mg,sg,exg,fc,chkf)
	local res
	...
	if sg:GetCount()<2 then
		...
		res=mg:IsExists(c74009824.filterchk,1,sg,tp,mg,sg,exg,fc,chkf)
	else
		res=aux.FCheckMixGoal(tp,sg,fc,true,true,chkf,c74009824.ffilter1,c74009824.ffilter2)
	end
	sg:RemoveCard(c)
	mg:Merge(rg)
	return res
...
function c74009824.fuscon(e,g,gc,chkf)
	...
	local mg=g:Filter(c74009824.ffilter,nil,c,SUMMON_TYPE_FUSION,tp)
	local exg=Group.CreateGroup()
	local tp=e:GetHandlerPlayer()
	local fc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	if fc and fc:IsHasEffect(81788994) and fc:IsCanRemoveCounter(tp,0x16,3,REASON_EFFECT) then
		exg=Duel.GetMatchingGroup(c74009824.exfilter,tp,0,LOCATION_MZONE,nil,g,c,SUMMON_TYPE_FUSION,tp)
		mg:Merge(exg)
	end
	if gc then
		return mg:IsContains(gc) and c74009824.filterchk(gc,tp,mg,Group.CreateGroup(),exg,c,chkf)
	end
	local sg2=Group.CreateGroup()
	return mg:IsExists(c74009824.filterchk,1,nil,tp,mg,Group.CreateGroup(),exg,c,chkf)
end
function c74009824.filterchk2(c,tp,mg,sg,exg,fc,chkf)
	return not exg:IsContains(c) and c74009824.filterchk(c,tp,mg,sg,exg,fc,chkf)
end
function c74009824.filterchk3(c,tp,mg,sg,exg,fc,chkf)
	return exg:IsContains(c) and c74009824.filterchk(c,tp,mg,sg,exg,fc,chkf)
end
function c74009824.fusop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	local tp=e:GetHandlerPlayer()
	local exg=Group.CreateGroup()
	local mg=eg:Filter(c74009824.ffilter,nil,c,SUMMON_TYPE_FUSION,tp)
	local p=tp
	local sfhchk=false
	local urg=Group.CreateGroup()
	if fc and fc:IsHasEffect(81788994) and fc:IsCanRemoveCounter(tp,0x16,3,REASON_EFFECT) then
		local sg=Duel.GetMatchingGroup(c74009824.exfilter,tp,0,LOCATION_MZONE,nil,eg,c,SUMMON_TYPE_FUSION,tp)
		exg:Merge(sg)
		mg:Merge(sg)
	end
	...
	while sg:GetCount()<2 do
		local tg=mg:Filter(c74009824.filterchk2,sg,tp,mg,sg,exg,c,chkf)
		local tg2=mg:Filter(c74009824.filterchk3,sg,tp,mg,sg,exg,c,chkf)
