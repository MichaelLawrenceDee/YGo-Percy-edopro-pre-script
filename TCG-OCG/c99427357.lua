...
function c99427357.spcfil(c)
	if not c:IsSetCard(0x2093) or not c:IsAbleToRemoveAsCost() then return false end
...
function c99427357.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return Duel.IsExistingMatchingCard(c99427357.spcfil,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,c) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c99427357.spcfil,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,c)
...
function c99427357.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
	if c:IsRelateToEffect(e) and tc and tc:IsRelateToEffect(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>1 
