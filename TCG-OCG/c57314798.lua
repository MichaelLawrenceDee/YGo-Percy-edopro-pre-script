...
function c57314798.xyztg(e,tp,eg,ep,ev,re,r,rp,chk,c,og,min,max)
	if og and not min then
		if og:GetCount()==2 or not og:IsExists(Card.IsHasEffect,1,nil,511002116) then
			og:KeepAlive()
			e:SetLabelObject(og)
			return true
		...
		if not og or min==99 then
			...
			while ct<2 do
				Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
				local sc=Group.SelectUnselect(mg:Filter(c57314798.xyzfilter2,g,mg,c,tp,matg,ct),g,tp,cancel,cancel)
