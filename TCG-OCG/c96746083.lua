	...
	if Duel.Destroy(g1,REASON_EFFECT)==2 then
		...
		if Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)==1 then
			...
			if Duel.IsPlayerAffectedByEffect(1-tp,69832741) then
				rg=Duel.GetMatchingGroup(c96746083.rmfilter,tp,0,LOCATION_MZONE,nil,tp)
			else
				rg=Duel.GetMatchingGroup(c96746083.rmfilter,tp,0,LOCATION_MZONE+LOCATION_GRAVE,nil,tp)
			end
