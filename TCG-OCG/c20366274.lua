...
function c20366274.FCheckGoal(tp,sg,fc)
	local g=Group.CreateGroup()
	return sg:IsExists(c20366274.FCheck,1,nil,sg,g,c20366274.ffilter1,c20366274.ffilter2) and Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0
		and (not aux.FCheckAdditional or aux.FCheckAdditional(tp,sg,fc))
end
function c20366274.FCheck(c,tp,mg,sg,fun1,fun2)
	if fun2 then
		sg:AddCard(c)
		mg:RemoveCard(c)
		local res=fun1(c,tp) and mg:IsExists(c20366274.FCheck,1,sg,mg,sg,fun2)
		sg:RemoveCard(c)
		mg:AddCard(c)
		return res
	else
...
function c20366274.filterchk(c,tp,mg,mg2,sg,fc)
	sg:AddCard(c)
	mg:RemoveCard(c)
	...
	if sg:GetCount()<2 then
		res=mg:IsExists(c20366274.filterchk,1,tp,mg,mg2,sg,fc)
	else
		res=c20366274.FCheckGoal(tp,sg,fc)
	end
	sg:RemoveCard(c)
	mg:AddCard(c)
	mg:Merge(rg)
...
function c20366274.fuscon(e,g,gc,chkf)
	...
	if gc then
		return mg:IsContains(gc) and c20366274.filterchk(gc,tp,mg,sg,Group.CreateGroup(),c)
	end
	local sg2=Group.CreateGroup()
	return mg:IsExists(c20366274.filterchk,1,nil,tp,mg,sg,sg2,c)
end
function c20366274.fusop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	if gc then
		...
		local g1=mg:FilterSelect(p,c20366274.filterchk,1,1,gc,tp,mg,exg,sg,c)
		...
		while sg:GetCount()<2 do
			Duel.Hint(HINT_SELECTMSG,p,HINTMSG_FMATERIAL)
			local tc=Group.SelectUnselect(mg:Filter(c20366274.filterchk,sg,tp,mg,exg,sg,c),sg,p)
