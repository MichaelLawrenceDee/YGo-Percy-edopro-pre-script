...
function c48424886.FCheckGoal(tp,sg,fc)
	local g=Group.CreateGroup()
	return sg:IsExists(c48424886.FCheck,1,nil,sg,g,c48424886.ffilter1,c48424886.ffilter2) and Duel.GetLocationCountFromEx(tp,tp,sg,fc)>0
		and (not aux.FCheckAdditional or aux.FCheckAdditional(tp,sg,fc))
end
function c48424886.FCheck(c,tp,mg,sg,fun1,fun2)
	if fun2 then
		sg:AddCard(c)
		mg:RemoveCard(c)
		local res=fun1(c,tp) and mg:IsExists(c48424886.FCheck,1,sg,mg,sg,fun2)
		sg:RemoveCard(c)
		mg:AddCard(c)
		return res
	else
...
function c48424886.filterchk(c,tp,mg,mg2,sg,fc)
	sg:AddCard(c)
	mg:RemoveCard(c)
	...
	if sg:GetCount()<2 then
		res=mg:IsExists(c48424886.filterchk,1,tp,mg,mg2,sg,fc)
	else
		res=c48424886.FCheckGoal(tp,sg,fc)
	end
	sg:RemoveCard(c)
	mg:AddCard(c)
	mg:Merge(rg)
...
function c48424886.fuscon(e,g,gc,chkf)
	...
	if gc then
		return mg:IsContains(gc) and c48424886.filterchk(gc,tp,mg,sg,Group.CreateGroup(),c)
	end
	local sg2=Group.CreateGroup()
	return mg:IsExists(c48424886.filterchk,1,nil,tp,mg,sg,sg2,c)
end
function c48424886.fusop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	if gc then
		...
		local g1=mg:FilterSelect(p,c48424886.filterchk,1,1,gc,tp,mg,exg,sg,c)
		...
		while sg:GetCount()<2 do
			Duel.Hint(HINT_SELECTMSG,p,HINTMSG_FMATERIAL)
			local tc=Group.SelectUnselect(mg:Filter(c48424886.filterchk,sg,tp,mg,exg,sg,c),sg,p)
