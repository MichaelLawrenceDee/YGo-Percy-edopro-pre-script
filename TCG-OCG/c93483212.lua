...
function c93483212.cfilter(c)
	if not c:IsSetCard(0x3042) or not c:IsType(TYPE_TUNER) or not c:IsAbleToRemoveAsCost() then return false end
...
function c93483212.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c93483212.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c93483212.cfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil)
