...
function c34898052.rfilter(c,att)
	if not c:IsAttribute(att) or not c:IsType(TYPE_MONSTER) or not c:IsAbleToRemoveAsCost() then return false end
...
function c34898052.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return g:IsExists(c34898052.filter,1,nil,g,Group.FromCards(e:GetHandler())) end
