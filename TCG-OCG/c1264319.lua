...
function c1264319.thfilter(c)
	if not c:IsSetCard(0x1047) or not c:IsType(TYPE_MONSTER) or not c:IsAbleToRemoveAsCost() then return false end
...
function c1264319.thcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c1264319.thfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c1264319.thfilter,tp,LOCATION_MZONE+LOCATION_GRAVE,0,1,1,nil)
